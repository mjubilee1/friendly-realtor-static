...
export const ValueContainer: React.ComponentType<ValueContainerProps<OptionType, true, GroupBase<OptionType>>> = (props) => {
  ...
  const defaultString = values.reduce((prev, next, index) => (index > 0 ? `${prev}, ${next.label}` : next.label), '');

  const valuesString = values.length === options.length && options.length > 1 ? selectAllString : values.length > 3 ? countString : defaultString;
