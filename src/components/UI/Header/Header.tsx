...
export default function Header({ as: HTag = 'h1', className, children, weight }: HeaderProps) {
  return <HTag className={`${className} ${getHeaderSize(HTag)} ${getHeaderFontWeight(weight)}`}>{children}</HTag>;
}
