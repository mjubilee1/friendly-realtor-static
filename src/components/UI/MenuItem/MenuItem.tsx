...
export default function MenuItem({ icon, disabled, items, className }: MenuItemProps) {
  ...
  return (
    <ul className={`flex flex-col gap-y-4 max-w-sm ${className}`}>
      {items.map(({ label, className: itemClassName, onClick }: ItemProps) => {
        ...
        return (
          <li className={`flex justify-between py-2 px-4 cursor-pointer text-sm ${itemClassName} ${disabledClass}`} key={uuidv4()} onClick={onClickProp}>
            <span>{label}</span>
