...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(({ type, color, children, icon, iconOnly, className, onClick, loading, href, target, disabled, size }: ButtonProps) => {
  ...
  return (
    <button type={btnType} className={`${combinedClassNames} focus:outline-none bg-center items-center cursor-pointer shadow-xs rounded-full`} {...onClickProp}>
      {icon && !loading && iconOnly ? <Icon name={icon.name} color={icon.color} solid={icon.solid} size={icon.size} className={`${icon.className || ''}`} /> : null}
      {icon && !loading && icon.position === 'left' && <Icon name={icon.name} color={icon.color} solid={icon.solid} size={icon.size} className={`${icon.className || ''}`} />}

      {!loading && children}
      {icon && !loading && icon.position === 'right' && <Icon name={icon.name} color={icon.color} solid={icon.solid} size={icon.size} className={`${icon.className || ''}`} />}
    </button>
  );
});
