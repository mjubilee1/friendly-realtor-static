...
export const FormText = React.forwardRef<HTMLInputElement, FormTextProps>((props: FormTextProps, ref) => {
  const { className, labelClassName, labelIcon, helperClassName, helperText, validationClassName, hideLabel, id, label, type = 'text', validationText, ...restProps } = props;
  ...
  return (
    <Group noGutter={hideLabel}>
      ...
      <div className={`${className} flex justify-end relative`}>
        <input {...restProps} className="block w-full rounded-lg" id={fieldId} ref={ref} type={type !== 'password' ? type : showPassword ? 'text' : 'password'} />
        {type === 'password' && (
          <button className="absolute pr-4 top-2 bg-transparent" onClick={toggleShowPassword} type="button">
